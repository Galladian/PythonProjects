...
with st.sidebar:
    ...
    for i, item in enumerate(st.session_state['items']):
        ...
        item['name'] = cols[0].text_input(f"n{i}", value=item['name'], key=f"n{i}", label_visibility="collapsed")
        item['percent'] = cols[1].number_input(f"p{i}", value=float(item['percent']), key=f"p{i}", label_visibility="collapsed")
...
def get_spin_data():
    ...
    # 270 aligns the 0-degree start point to the top
...
def build_svg():
    items = st.session_state['items']
    total_p = sum(x['percent'] for x in items) if sum(x['percent'] for x in items) > 0 else 1
...
with col_left:
    # We use st.fragment (if on newer streamlit) or just a container
    # Removing 'key' from components.html to fix your error
    html_code = f"""
    ...
    """
    # The fix: remove 'key' and use a standard call
...
with col_right:
    ...
    if result:
        ...
        st.info("The wheel is ready. Click SPIN!")